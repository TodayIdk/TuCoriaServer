...
function createRoom() {
    ...
    // Создаём физический мир для комнаты
    ...
    const room = {
        ...
        rigidBodies: new Map(),    // blockId -> RAPIER.RigidBody
        nextPlayerId: 1,
        nextBlockId: 1,
...
function createRigidBodyForBlock(room, block) {
    ...
    // Коллайдер в зависимости от формы
    let colliderDesc;
    const hx = block.size.x * 0.5;
    const hy = block.size.y * 0.5;
    const hz = block.size.z * 0.5;
    ...
    switch (block.shape) {
        ...
        case 2: // Cylinder
        case 4: // Tube
            colliderDesc = RAPIER.ColliderDesc.cylinder(hy, Math.max(hx, hz));
            break;
        case 3: // Wedge (упрощаем до кубика)
            colliderDesc = RAPIER.ColliderDesc.cuboid(hx, hy, hz);
            break;
        default:
            colliderDesc = RAPIER.ColliderDesc.cuboid(hx, hy, hz);
    ...
    if (block.canCollide === 0) {
        // Убираем коллизию но оставляем body для позиции
...
// ═══════════════════════════════════════
//  Физический tick (60 Hz)
// ═══════════════════════════════════════
const PHYSICS_TICK_MS = 1000 / 60;   // 60 Hz симуляция
const NET_SYNC_MS = 50;              // 20 Hz синхронизация по сети

function physicsStep() {
    const now = Date.now();
    for (const room of rooms.values()) {
        if (!room.world) continue;
        room.world.step(room.eventQueue);

        // Синхронизация 20 раз/сек
...
function syncPhysicsToClients(room) {
    ...
    // Отправляем пакет S_PHYSICS_STATE
    const w = new Writer();
...
setInterval(physicsStep, PHYSICS_TICK_MS);
...
function spawnBot(name = 'Bot') {
    ...
    console.log(`[BOT] Spawned ${name} (pid=${playerId})`);
    ...
    ws.on('message', (data) => {
        try {
            ...
            if (type === PT.C_HELLO) {
                ...
                console.log(`[WS] ${name} joined ${room.id} as pid=${playerId}`);
            ...
            if (type === PT.C_BLOCK_SPAWN) {
                ...
                room.rigidBodies.set(b.blockId, createRigidBodyForBlock(room, b));
            ...
            if (type === PT.C_BLOCK_UPDATE) {
                ...
                if (room.blocks.has(b.blockId)) {
                    ...
                    // Пересоздаём физическое тело если shape/size/anchored изменились
                    const needRecreate = 
                    ...
                    if (needRecreate) {
                        ...
                        // Просто обновляем позицию/вращение
...
// ═══════════════════════════════════════
//  Инициализация rapier и запуск сервера
// ═══════════════════════════════════════
(async () => {
    await RAPIER.init();
